...
# FROM python:3.9-alpine3.13 => python: ten image, 3.9-alpine3.13: name of the tag, 
# alpine is lightweight version of linux, it 's ideal for unning docker container because it very stripped back, 
...
# COPY ./requirements.txt /tmp/requirements.txt: our requirements.txt from our local machine to /tmp/requirements.txt
...
# ENV PATH="/py/bin:$PATH" => update environment inside the image and we're updating the path enviroment variable
